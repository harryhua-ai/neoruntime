...
// ── Constants ─────────────────────────────────────────────────────────
// UI shows 1–4x (product scale). API still uses level 0–1 → motor / optical ~1–2.88x.

const ZOOM_DISPLAY_MIN = 1.0;
const ZOOM_DISPLAY_MAX = 4.0;
...
export default function LensControl() {
  ...
  const [isOneshotAF, setIsOneshotAF] = useState(false);
  ...
  const zLevel = useMemo(
    () => (lensStatus ? zoomLevelFromStatus(lensStatus) : 0),
    [lensStatus]
  );
  ...
  const isZoomBusy =    setZoomLevel.isPending || isMotorInitializing || hasMotorError;
  const isFocusBusy =    setFocusLevel.isPending || isMotorInitializing || hasMotorError;
  ...
  const handleOneshotAutofocus = async () => {
    ...
    try {
      ...
      toast.success(
        t('sys.ptz.oneshot_af_done', 'One-shot autofocus completed')
      );
    } catch (error: unknown) {
      ...
      setIsOneshotAF(false);
      await refetchLensStatus();
    }
  };

  const handleResetZoom = async () => {
    setZoomPercent(0);
    await setZoomLevel.mutateAsync(0);
  };
      ...
      <CardContent className="space-y-5 p-4">
        ...
        <MotorAxisControl
          ...
          onCommit={async level => {
            await setZoomLevel.mutateAsync(level);
          }}
          busy={isZoomBusy}
