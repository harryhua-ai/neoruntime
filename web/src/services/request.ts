...
const longTimeTaskMap: Record<string, number> = {
  '/api/v1/system/ota/parse': 300000, // 5 min — upload
  '/api/v1/system/ota/install': 300000, // 5 min — upgrade
  '/api/v1/system/os-upgrade/upload': 600000, // 10 min — upload
  '/api/v1/system/os-upgrade/validate': 0,
  '/api/v1/apps/upload-image': 300000, // 5 min — image upload
...
// Debounce repeated 500 toasts to avoid spamming users when backend is unstable.
const debouncedHttp500Error = debounce(
  2000,
  (msg: string) => {
    toast.error(msg);
  },
...
let networkErrorToastSuppressDepth = 0;

/** Suppress global network-error toasts until the returned function is called. */
export function enterNetworkErrorToastSuppress() {
  networkErrorToastSuppressDepth += 1;
  return () => {
    networkErrorToastSuppressDepth = Math.max(
      0,
      networkErrorToastSuppressDepth - 1
    );
  };
...
function shouldSuppressNetworkErrorToast(config?: {
  ...
  if (networkErrorToastSuppressDepth > 0) return true;
...
request.interceptors.response.use(
  response => {
    ...
    if (data && typeof data === 'object') {
      ...
      if (data.code === 500 || isInternalServerErrorMessage(finalMsg)) {
        debouncedHttp500Error(finalMsg);
      } else {
  ...
  error => {
    ...
    switch (status) {
      ...
      case 500:
        debouncedHttp500Error(i18n.t('errors.http.500', { ns: 'errors' }));
        return Promise.reject(error.response);
