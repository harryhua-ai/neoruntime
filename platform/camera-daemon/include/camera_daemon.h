...
public:
    ...
    // does not hold op_mu_; it only serializes its const& argument, so no lock is
    // required. Both helpers live under #ifdef HAS_GRPC (same as the OSD/privacy
    // helpers); the persist call site inside the unguarded set_transform_config
...
private:
    ...
    grpc::Service* lens_hal_service_ = nullptr;
    ...
    // Privacy mask items cache — HAL HalPrivacyMaskItem* must remain valid
    // after set_privacy_mask_config returns (pointers into this vector).
