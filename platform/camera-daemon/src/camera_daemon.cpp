...
bool CameraDaemon::init(const DaemonConfig& config) {
    ...
    // Wire VideoSource callbacks to FrameRouter
    ...
    for (auto& slot : video_source_->streams()) {
        ...
        video_source_->set_frame_callback(slot.name,
            [this, dispatch_name](const std::string& stream_name, HalFrameBuffer* frame) {
...
bool CameraDaemon::set_transform_config(const aipc::camera::TransformConfig& config) {
    ...
    ic.flip_direction = static_cast<HalFlipDirection>(config.flip());
...
bool CameraDaemon::get_privacy_mask_config(aipc::camera::PrivacyMaskConfig& config) {
    ...
    for (uint32_t i = 0; i < pm.item_count; ++i) {
...
bool CameraDaemon::set_privacy_mask_config(const aipc::camera::PrivacyMaskConfig& config) {
    ...
    // Now set id/name pointers from cached strings (must outlive HAL call)
    // We need stable string storage — use a separate vector
    // Actually, we need the C strings to remain valid. Store them alongside.
    // Use static thread-local string vectors for id/name since HAL doesn't copy them.
    static thread_local std::vector<std::string> pm_ids, pm_names;
    pm_ids.clear();
    pm_names.clear();
    pm_ids.reserve(config.regions_size());
    pm_names.reserve(config.regions_size());
    for (int i = 0; i < config.regions_size(); ++i) {
        pm_ids.push_back(config.regions(i).id());
        pm_names.push_back(config.regions(i).name());
        cached_pm_items_[i].id = pm_ids.back().c_str();
        cached_pm_items_[i].name = pm_names.back().c_str();
    }
...
bool CameraDaemon::start_dpm_worker(const aipc::camera::PrivacyMaskConfig& config) {
    ...
    // hailortcli parse-hef reports "number of classes: 4"; apps/model-showcase
    ...
    if (has_label("license_plate")) {
        ...
        // keep_labels empty => keep every detection (the model only emits plates).
        s.post_json = "/data/aipc-data/models/detection/tiny_yolov4_license_plates.json";
        specs.push_back(std::move(s));
    }
...
void CameraDaemon::start_grpc_server() {
    ...
    if (!config_.lens_bridge_lib.empty()) {
        ...
        lens_hal_service_ = CreateLensHalService(lens_cfg);
        if (lens_hal_service_) {
            builder.RegisterService(lens_hal_service_);
...
void CameraDaemon::stop_grpc_server() {
    ...
    delete lens_hal_service_;
    lens_hal_service_ = nullptr;
...
bool CameraDaemon::init_media() {
    ...
    if (config_.media_config_path.empty() && config_.media_config_json.empty()) {
        HAL_LOG_INFO("CameraDaemon: No media config provided, skipping media pipeline");
        return true;
    }
...
void CameraDaemon::get_stream_status(aipc::camera::GetStreamStatusResponse& response) {
    ...
    for (const auto& ec : config_.encoders) {
        ...
        info->set_codec(ec.codec);
        info->set_width(ec.width);
        info->set_height(ec.height);
        info->set_fps(ec.fps);
        info->set_bitrate_bps(ec.bitrate);
        info->set_gop(ec.gop);
        ...
        if (media_ops->get_video_list && video_source_) {
            ...
            if (media_ops->get_video_list(media_ctx_, &video_list, &video_count) >= 0
                ...
                for (auto& slot : video_source_->streams()) {
                    ...
                    video_source_->set_frame_callback(slot.name,
                        [this, dispatch_name](const std::string& sn, HalFrameBuffer* frame) {
                            frame_router_->on_frame_arrived(dispatch_name, frame);
                        });
                }
                for (auto& slot : video_source_->streams()) {
            ...
            if (slot.name == stream_id) {
                video_source_->set_frame_callback(slot.name,
                    [this, stream_id](const std::string&, HalFrameBuffer* frame) {
                        if (frame_router_) frame_router_->on_frame_arrived(stream_id, frame);
                    });
                video_source_->start_stream(slot.name);
                break;
            }
...
bool CameraDaemon::switch_profile(const std::string& profile_name, std::string* message) {
    ...
    int ret = media_ops->switch_profile(media_ctx_, profile_name.c_str());
    if (ret < 0) {
        ...
        // Attempt rollback to previous profile
        if (!prev_profile.empty()) {
            int rb = media_ops->switch_profile(media_ctx_, prev_profile.c_str());
            if (rb < 0) {
