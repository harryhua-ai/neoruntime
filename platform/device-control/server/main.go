...
type DeviceControlServer struct {
	...
	autofocusMu       sync.Mutex
	autofocusInFlight bool
...
func (s *DeviceControlServer) zoomRatioFromPos(zoomPos int32) float32 {
	if s.halLens != nil {
		return s.halLens.AF0832PosToRatio(zoomPos)
	}
	lim := s.halLens.ZoomLimits()
	minRatio, maxRatio := s.lensZoomRatioBounds()
	den := lim.MaxPos - lim.MinPos
	if den <= 0 {
		return minRatio
	}
	level := float32(zoomPos-lim.MinPos) / float32(den)
	if level < 0 {
		level = 0
	}
	if level > 1 {
	...
	return minRatio + (maxRatio-minRatio)*level
}

func (s *DeviceControlServer) runAutofocusAtCurrentZoom() error {
	if s.halLens == nil {
		return fmt.Errorf("lens HAL not initialized")
	}
	if err := s.ensureLensBootstrapped(); err != nil {
	...
	freshState, err := s.halLens.StateGet()
	if err != nil {
		return fmt.Errorf("autofocus state_get failed: %w", err)
	}
	originalZoomPos := freshState.ZoomPos
	currentZoomRatio := s.zoomRatioFromPos(originalZoomPos)
	focusDistanceM := s.lensAutofocusDistanceM()

	logger.Info("autofocus: zoomPos=%d zoomRatio=%.3f focusDistance=%.2fm", originalZoomPos, currentZoomRatio, focusDistanceM)

	if err := s.halLens.StopAndWaitAll(2 * time.Second); err != nil {
		return fmt.Errorf("autofocus pre-stop failed: %w", err)
	}

	doGoto := func() error {
		return s.halLens.AF0832GotoRatioDistance(currentZoomRatio, focusDistanceM)
	}
	if err := doGoto(); err != nil {
		if isRecoverable(err) {
			logger.Warn("autofocus transient error, trying recover+retry once: %v", err)
			if re := s.recoverLensLink(); re != nil {
				return fmt.Errorf("autofocus recover failed: %w", re)
			}
			if err2 := doGoto(); err2 != nil {
				return fmt.Errorf("autofocus goto-ratio retry failed: %w", err2)
			}
		} else {
			return fmt.Errorf("autofocus goto-ratio failed: %w", err)
		}
	}
	if err := s.halLens.WaitFocusStopped(15 * time.Second); err != nil {
		return fmt.Errorf("wait autofocus focus stop failed: %w", err)
	}
	_ = s.halLens.WaitZoomStopped(5 * time.Second)

	afterState, err := s.halLens.StateGet()
	if err != nil {
		logger.Warn("autofocus: post state_get failed: %v", err)
		return nil
	}
	zoomDrift := afterState.ZoomPos - originalZoomPos
	if zoomDrift < 0 {
		zoomDrift = -zoomDrift
	}
	const zoomDriftThreshold = 50
	if zoomDrift > zoomDriftThreshold {
		logger.Info("autofocus: zoom drifted %d→%d (delta=%d), restoring", originalZoomPos, afterState.ZoomPos, zoomDrift)
		if err := s.halLens.ZoomAbs(2000, originalZoomPos); err != nil {
			logger.Warn("autofocus: zoom restore failed: %v", err)
		} else {
			_ = s.halLens.WaitZoomStopped(10 * time.Second)
		}
	}
	return nil
}

func (s *DeviceControlServer) triggerAutofocusAsync(reason string) {
	if s.halLens == nil {
		return
	}
	s.autofocusMu.Lock()
	if s.autofocusInFlight {
		s.autofocusMu.Unlock()
		logger.Debug("autofocus task already running, skip trigger (%s)", reason)
		return
	}
	s.autofocusInFlight = true
	s.autofocusMu.Unlock()

	go func() {
		defer func() {
			s.autofocusMu.Lock()
			s.autofocusInFlight = false
			s.autofocusMu.Unlock()
		}()
		if !s.autofocusEnabled {
			logger.Debug("autofocus disabled before async trigger (%s) execution", reason)
			return
		}
		if err := s.runAutofocusAtCurrentZoom(); err != nil {
			if isRecoverable(err) {
				logger.Info("autofocus async trigger (%s) skipped due transient HAL error: %v", reason, err)
				return
			}
			logger.Warn("autofocus async trigger (%s) failed: %v", reason, err)
			return
		}
		logger.Info("autofocus async trigger (%s) completed", reason)
	}()
}
...
func (s *DeviceControlServer) Zoom(ctx context.Context, req *pb.ZoomRequest) (*pb.Status, error) {
	...
	if s.autofocusEnabled {
		s.triggerAutofocusAsync("zoom_speed")
	}
...
func (s *DeviceControlServer) SetAutofocus(ctx context.Context, req *pb.AutofocusRequest) (*pb.Status, error) {
	...
	if req.Enable {
		s.autofocusEnabled = true
		logger.Info("Autofocus enabled; triggering initial autofocus")
		s.triggerAutofocusAsync("autofocus_enabled")
		return &pb.Status{Success: true}, nil
	}
...
func (s *DeviceControlServer) SetZoomLevel(ctx context.Context, req *pb.ZoomLevelRequest) (*pb.Status, error) {
	...
	if s.autofocusEnabled {
		s.triggerAutofocusAsync("set_zoom_level")
	}
