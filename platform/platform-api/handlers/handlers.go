...
func NewAPIHandlers(clients *GRPCClients, modelStorage string, modelStore *storage.ModelStorage, db *gorm.DB, authUser, authPass, tokenKey string, eventLogger *eventLoggerPkg.Logger) *APIHandlers {
	h := &APIHandlers{
		grpcClients:  clients,
	...
	if db != nil {
		...
		h.configMgr.Register("auth", auth.New(""))
	}
	return h
}
