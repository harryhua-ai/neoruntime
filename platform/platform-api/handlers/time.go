...
func NewTimeHandler(configPath string, eventLogger *eventLoggerPkg.Logger, configMgr *config.Manager) *TimeHandler {
	return &TimeHandler{
...
func (h *TimeHandler) SetNTPConfig(c *gin.Context) {
	...
	if actualNTPState != req.Enabled {
		logger.Error("NTP state verification failed: expected %v, got %v", req.Enabled, actualNTPState)
		Resp(c).FailMsg(CodeServiceError, fmt.Sprintf("NTP state verification failed: expected %v, got %v", req.Enabled, actualNTPState))
		return
	}
	...
	logger.Info("NTP configuration updated: enabled=%v, server=%s", req.Enabled, serverLog)
