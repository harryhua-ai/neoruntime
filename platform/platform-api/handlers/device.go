...
func (h *APIHandlers) OneshotAutofocus(c *gin.Context) {
	...
	enableCtx, enableCancel := context.WithTimeout(context.Background(), 120*time.Second)
	defer enableCancel()
	resp, err := client.SetAutofocus(enableCtx, &devicepb.AutofocusRequest{Enable: true})
	if err != nil {
		Resp(c).FailMsg(CodeDeviceError, "enable autofocus: "+err.Error())
		return
	}
	if !resp.GetSuccess() {
		Resp(c).FailMsg(CodeDeviceError, "enable autofocus: "+resp.GetMessage())
		return
	}

	// Wait for motors to start moving (state leaves idle=1)
	...
	for time.Now().Before(deadline) {
		...
		st, err := client.GetLensStatus(statusCtx, &devicepb.Empty{})
		statusCancel()
		if err != nil {
			continue
		}
		if st.GetZoomState() == 0 || st.GetFocusState() == 0 {
			break
		}
		if st.GetZoomState() == 1 && st.GetFocusState() == 1 {
			settled = true
			break
		}
	}
	...
	defer disableCancel()
	_, _ = client.SetAutofocus(disableCtx, &devicepb.AutofocusRequest{Enable: false})
	...
	if !settled {
		Resp(c).FailMsg(CodeDeviceError, "autofocus did not settle in time")
		return
	}

	statusCtx2, statusCancel2 := context.WithTimeout(context.Background(), 5*time.Second)
	defer statusCancel2()
	finalStatus, err := client.GetLensStatus(statusCtx2, &devicepb.Empty{})
	if err != nil {
		Resp(c).OK(gin.H{"success": true})
		return
	}
	Resp(c).OK(gin.H{
		"success":   true,
		"zoom_pos":  finalStatus.GetZoomPos(),
		"focus_pos": finalStatus.GetFocusPos(),
	})
}
