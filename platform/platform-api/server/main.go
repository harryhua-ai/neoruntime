...
func NewPlatformAPIServer(cfg *Config) (*PlatformAPIServer, error) {
	...
	server.httpServer = &http.Server{
		...
		ReadTimeout:  5 * time.Minute, // Allow large file uploads (OTA firmware)
...
func (s *PlatformAPIServer) setupRoutes() {
	...
	}, modelStoragePath, modelStore, s.db, s.config.Auth.Username, s.config.Auth.Password, s.config.Auth.TokenKey, s.eventLogger)
	...
	systemHandler := handlers.NewSystemHandlers("", apiHandlers.ConfigManager())
	...
	api.GET("/system/ota/status", systemHandler.OTAGetStatus)
	...
	api.GET("/system/os-upgrade/status", osUpgradeHandler.Status)
	...
	// Time routes
	timeHandler := handlers.NewTimeHandler("", s.eventLogger, apiHandlers.ConfigManager())
	s.engine.POST("/api/v1/system/time/sync-from-client", timeHandler.SyncFromClient) // Auto-sync needs no auth (pre-login)
	...
	// File Management
	fileHandler := handlers.NewFileHandler(nil, constants.RootPath()) // default: /data/aipc, /tmp
...
func (s *PlatformAPIServer) buildGyroSource() gyro.Source {
	...
	s.gyroCancel = cancel
	go src.Start(ctx)
	logger.Info("gyro source started (poll_rate_hz=%d, fusion_alpha=%.3f, calibrate=%v, calibration_ms=%d)",
		cfg.PollRateHz, cfg.FusionAlpha, cfg.CalibrateBias, cfg.CalibrationMs)
	return src
}
