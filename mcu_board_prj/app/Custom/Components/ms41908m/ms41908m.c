                ...
                if (s_instance.focus_run_config.is_valid) {
                    ...
                    if (s_instance.focus_run_config.unload_psum > 0) {
                        ...
                        if (ret != SYS_OK) {
                            ...
                            // WIC_LOGI("[ms41908m] Focus step updated: intct=%d, load_psum=%c%d, unload_psum=%c%d",
                            //          s_instance.focus_run_config.intct, s_instance.focus_run_config.direction == 1 ? '+' : '-', s_instance.focus_run_config.load_psum,
                            //          s_instance.focus_run_config.direction == 1 ? '+' : '-', s_instance.focus_run_config.unload_psum);
                        }
                    } else {
                        /* Focus completed */
                        ms41908m_motor_stop_impl(MS41908M_TYPE_FOCUS);
                        if (s_rst.phase != MS41908M_RZ_IDLE &&
                            ...
                            /* In reset-zero flow, COMPLETED is an internal phase signal; do not
                             * forward as external app event to avoid false "run done". */
                ...
                if (s_instance.zoom_run_config.is_valid) {
                    ...
                    if (s_instance.zoom_run_config.unload_psum > 0) {
                        ...
                        if (ret != SYS_OK) {
                            ...
                            ms41908m_port_output_vd(MS41908M_TYPE_ZOOM);
                            // WIC_LOGI("[ms41908m] Zoom step updated: intct=%d, load_psum=%c%d, unload_psum=%c%d",
                            //          s_instance.zoom_run_config.intct, s_instance.zoom_run_config.direction == 1 ? '+' : '-', s_instance.zoom_run_config.load_psum,
                            //          s_instance.zoom_run_config.direction == 1 ? '+' : '-', s_instance.zoom_run_config.unload_psum);
                        }
                    } else {
                        /* Zoom completed */
                        ms41908m_motor_stop_impl(MS41908M_TYPE_ZOOM);
...
int ms41908m_zoom_run(uint16_t pps, int32_t micro_steps)
{
    ...
    /* Under lock: mutual exclusion with focus + reset */
