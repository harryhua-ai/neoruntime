 ...
 */
#ifndef HOST_LINK_PROTO_H
#define HOST_LINK_PROTO_H
...
/**
 * Command IDs (uint16_t on wire). 0–0x000F reserved / well-known; 0x0010+ map to ne503 MCU (bsp_ctrl + RTC).
 * Linux host: send REQUEST with optional payload; MCU replies RESPONSE (see each command).
 */
typedef enum {
    ...
    /** MCU -> host EVENT: raw RS485 RX bytes. */
    HOST_LINK_CMD_EV_RS485_RX = 0x0041,
    /** MCU -> host EVENT: lens async completion/reset result (payload @ref host_link_lens_evt_t). */
    HOST_LINK_CMD_EV_LENS = 0x004C,

    /* OTA:
     * Single-slot XIP: APP must NOT download/write firmware.
     * Host requests APP to set a bootloader tag and reboot; boot handles Ymodem upgrade.
     */
...
/** GET_VERSION response (same idea as ms_bridging_version_t). */
typedef struct {
    int32_t major;
    int32_t minor;
    int32_t patch;
...
/** RTC: BIN year 0–99, month 1–12, date 1–31, weekday 1–7, 24h time. */
typedef struct {
    uint8_t year;
    uint8_t month;
    uint8_t day;
...
typedef struct {
    uint8_t led_id; /**< bsp_led_t: 0 near … 4 ir1 */
    uint8_t duty;   /**< 0–100 */
} host_link_led_set_t;

typedef struct {
    uint8_t channel; /**< bsp_alarm_out_t or bsp_wiegand_t: 0–1 */
    uint8_t enable;
} host_link_ch_enable_t;

/** EVENT EV_ALARM_IN: channel = bsp_alarm_in_t, level after edge (0=low / 1=high). */
typedef struct {
    uint8_t channel;
    uint8_t level;
} host_link_alarm_in_evt_t;

/** PD_GET / TEMP_GET response: ADC mV + scaled value (lux*1000 or °C*1000). */
typedef struct {
    uint16_t mv;
    int32_t milli;
} host_link_adc_milli_t;

/** Many handlers return int32_t sys_err_t-style status only. */
typedef struct {
    int32_t status;
} host_link_status_t;
...
typedef struct {
    uint32_t event;   /**< MS41908M_EVT_* value */
    int32_t result;   /**< SYS_OK or error code */
    int32_t zoom_pos; /**< snapshot position when event raised */
    int32_t focus_pos;
