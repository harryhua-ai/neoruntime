                                             ...
                                             const config_profile_t &target_profile)
{
    ...
    if (cfg.contains("default_profile"))
...
static int hailo15_media_init(const HalMediaConfig *config, void **media_ctx_return)
{
    ...
    else if (config->config_path && config->config_path[0])
    {
        json = hailo15_read_file(config->config_path);
        if (json.empty())
        {
            return HAL_ERR_INVALID_ARG;
        }
 ...
 */
static int hailo15_media_switch_profile(void *media_ctx, const char *profile_name)
{
    auto *hm = static_cast<HalMediaContext *>(media_ctx);
    auto *priv = hailo15_media_priv_from_hal(media_ctx);
    ...
    if (r != MEDIA_LIBRARY_SUCCESS)
    {
        ...
        HAL_LOG_WARNING(
            ...
            restricted ? " -> PROFILE_IS_RESTRICTED (thermal gates AI Denoise; cool to FULL_PERFORMANCE "
                       ...
                       : "");
        if (had_bridge && disconnect_before)
        {
            const int ce = connect_encoders(priv);
            if (ce != HAL_OK)
            {
    ...
    if (had_bridge)
    {
        ...
        {
            HAL_LOG_INFO(
                "hailo15_media: switch_profile(%s): layout changed pre-set_profile — HAL recycle (stop_pipeline, "
                "bridges, start_pipeline)",
                profile_name);
            media_library_return sr = priv->media_lib->stop_pipeline();
            if (sr != MEDIA_LIBRARY_SUCCESS)
            {
                                ...
                                const HalMediaImageConfig *cfg)
{
    ...
    /* 1. Apply encoder overrides first (writes correct non-rotated dims). */
    std::string patched_json = priv->stored_config_json;
    if (!priv->encoder_overrides_json.empty())
        apply_encoder_overrides(patched_json, priv->encoder_overrides_json.c_str());
    else
...
static int hailo15_media_dynamic_change_image_config(void *media_ctx, const HalMediaImageConfig *cfg)
{
    ...
    if (prev_portrait != new_portrait)
    {
        ...
        /* Detect large encoders that need the full-reinit path. */
        bool needs_full_reinit = false;
        for (auto &kv : p.encoded_output_streams)
        {
            uint32_t w = 0, h = 0;
...
static int hailo15_media_override_stream_params(void *media_ctx, const HalStreamOverrideBatch *batch)
{
    ...
    if (full_reinit_required)
    {
        ...
        std::string new_json = hailo15_generate_pipeline_config_json(priv->stored_config_json, &fake_reconfig);
        if (new_json.empty())
        {
    ...
    const HalPipelineReconfig *reconfig)
{
    ...
    /* Determine the current (or default) profile name */
    std::string prof_name;
    if (cfg.contains("default_profile"))
    {
        prof_name = cfg["default_profile"].get<std::string>();
...
static int hailo15_media_reconfigure_pipeline(void *media_ctx, const HalPipelineReconfig *reconfig)
{
    ...
    if (full_reinit_required)
    {
        HAL_LOG_INFO("hailo15_media: reconfigure_pipeline: stream buffer layout change detected, full reinit");
        std::string new_json = hailo15_generate_pipeline_config_json(priv->stored_config_json, reconfig);
        if (new_json.empty())
        {
