...
/* Caller must hold h->lock. For async completion, unlocks before done_cb. */
static int add_ack_frame_locked(struct host_link_handler *h, host_link_frame_t *f)
{
    if (f->header.type == HOST_LINK_TYPE_RESPONSE) {
        for (uint16_t i = 0; i < HOST_LINK_CFG_ASYNC_MAX; i++) {
            if (h->async_slot[i].used && h->async_slot[i].id == f->header.frame_id && h->async_slot[i].cmd == f->header.cmd) {
...
static int send_wire(struct host_link_handler *h, host_link_header_t *hdr, const uint8_t *payload, uint16_t pay_len)
{
    ...
    uint16_t total = (uint16_t)(HOST_LINK_HEADER_WIRE_LEN + pay_len + 2u);
                        ...
                        host_link_frame_t *out)
{
    ...
    for (;;) {
        ...
        host_link_port_mutex_lock(h->lock);
        for (uint16_t i = 0; i < HOST_LINK_CFG_ACK_SLOTS; i++) {
            if (h->ack_slot[i].is_valid && h->ack_slot[i].header.frame_id == frame_id && h->ack_slot[i].header.cmd == cmd
                && h->ack_slot[i].header.type == typ) {
        ...
        (void)host_link_port_sem_wait(h->ack_sem, HOST_LINK_CFG_ACK_WAIT_SLICE_MS);
    }
}

/* Frame id allocator is defined later; forward declare for timeout API. */
static uint16_t alloc_frame_id(struct host_link_handler *h);

static int wait_for_ack_timeout(struct host_link_handler *h, host_link_frame_type_t typ, uint16_t cmd, uint16_t frame_id,
                                host_link_frame_t *out, uint32_t timeout_ms)
{
    uint32_t t0 = host_link_port_tick_ms();
    for (;;) {
        if (!h->ready) {
        ...
        uint32_t now = host_link_port_tick_ms();
        if (host_link_port_tick_diff_ms(t0, now) >= timeout_ms) {
            return HOST_LINK_ERR_TIMEOUT;
        }
        (void)host_link_port_sem_wait(h->ack_sem, HOST_LINK_CFG_ACK_WAIT_SLICE_MS);
    }
...
int host_link_request_timeout(host_link_handler_t *h, uint16_t cmd, const void *req_data, uint16_t req_len, void **resp_data,
                              uint16_t *resp_len, uint32_t timeout_ms)
{
    ...
    while (retry < HOST_LINK_CFG_REQUEST_RETRY && ret != HOST_LINK_OK) {
        ...
        ret = wait_for_ack_timeout(h, HOST_LINK_TYPE_RESPONSE, cmd, fid, &ack, timeout_ms);
...
void host_link_poll(struct host_link_handler *h)
{
    ...
    for (uint16_t i = 0; i < HOST_LINK_CFG_ASYNC_MAX; i++) {
        if (h->async_slot[i].used && host_link_port_tick_diff_ms(h->async_slot[i].started_ms, now) >= HOST_LINK_CFG_ACK_TIMEOUT_MS) {
            host_link_request_done_fn cb = h->async_slot[i].cb;
            void *u = h->async_slot[i].cb_user;
            h->async_slot[i].used = 0;
            host_link_port_mutex_unlock(h->lock);
    ...
    if (h->ready) {
        (void)host_link_port_sem_wait(h->notify_sem, HOST_LINK_CFG_POLL_WAIT_MS);
    } else {
...
int host_link_request(host_link_handler_t *h, uint16_t cmd, const void *req_data, uint16_t req_len, void **resp_data,
                      uint16_t *resp_len)
{
    return host_link_request_timeout(h, cmd, req_data, req_len, resp_data, resp_len, HOST_LINK_CFG_ACK_TIMEOUT_MS);
}
